...
io.on("connection", (socket) => {
  ...
  socket.on("join", (data: { username: string }) => {
    ...
    console.log(
      `${username} joined the chat room, current online users: ${users.size}`,
    );
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (user) {
      ...
      const leaveMessage = createSystemMessage(
        `${user.username} left the chat room`,
      );
      io.emit("user-left", {
      ...
      console.log(
        `${user.username} left the chat room, current online users: ${users.size}`,
      );
    } else {
